...
def validate_payload(payload, token_value):
    ...
    if not data:
        ...
        try:
            ...
            errors.append(
                f"email_timestream is not a valid integer: {data.get('email_timestream')}"
            )
...
def message():
    ...
    if errors:
        return (
            jsonify(
                {"status": "rejected", "errors": errors, "token_param": TOKEN_VALUE}
            ),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "status": "error",
                    "message": f"SQS send failed: {str(e)}",
                    "token_param": TOKEN_VALUE,
                }
            ),
            500,
